...
public class FormController {
    ...
    @ApiOperation(value = "首页人员查询", notes = "首页人员查询", httpMethod = "GET", tags = "首页人员查询接口")
    @GetMapping(value = "/formInfo")
    @ResponseBody
    public String getFormInfo(HttpServletRequest request) {
        try {
            //从请求的header中取出当前登录的登录
            SYS_USER user = UserManager.getUserToken(request, userService, unitService, peopleService);
            if (user != null) {
                FormModel model=new FormModel();
                List<SYS_People> incumbents=peopleService.selectIncumbentPeoplesByUnitId(user.getUnitId(),"在职");
                if (incumbents!=null){
                    model.setIncumbent(incumbents.size());
                }
                List<SYS_People> leavePeoples=peopleService.selectIncumbentPeoplesByUnitId(user.getUnitId(),"离职");
                if (leavePeoples!=null){
                    model.setLeavePeople(leavePeoples.size());
                }
                List<SYS_People> turnPeople=peopleService.selectTrunPeoplesByUnitId(user.getUnitId());
                if (turnPeople!=null){
    ...
    @ApiOperation(value = "首页职级查询", notes = "首页职级查询", httpMethod = "GET", tags = "首页职级查询接口")
    @GetMapping(value = "/formRankInfo")
    @ResponseBody
    public String getFormRankInfo(HttpServletRequest request) {
        try {
            //从请求的header中取出当前登录的登录
            SYS_USER user = UserManager.getUserToken(request, userService, unitService, peopleService);
            if (user != null) {
    ...
    @ApiOperation(value = "性别统计查询", notes = "性别统计查询", httpMethod = "GET", tags = "性别统计查询接口")
    @GetMapping(value = "/sexInfo")
    @ResponseBody
    public String getSexData(HttpServletRequest request) {
        try {
            //从请求的header中取出当前登录的登录
            SYS_USER user = UserManager.getUserToken(request, userService, unitService, peopleService);
            if (user != null) {
    ...
    @ApiOperation(value = "党员统计查询", notes = "党员统计查询", httpMethod = "GET", tags = "党员统计查询接口")
    @GetMapping(value = "/partyInfo")
    @ResponseBody
    public String getPartyData(HttpServletRequest request) {
        try {
            //从请求的header中取出当前登录的登录
            SYS_USER user = UserManager.getUserToken(request, userService, unitService, peopleService);
            if (user != null) {
                FormModel model=new FormModel();
                int notP=0;
                List<SYS_People> party=peopleService.selectPartyPeoplesByUnitId(user.getUnitId(),"中共党员");
                if (party!=null){
    ...
    @ApiOperation(value = "职务统计查询", notes = "职务统计查询", httpMethod = "GET", tags = "职务统计查询接口")
    @GetMapping(value = "/dutyInfo")
    @ResponseBody
    public String getDutyData(HttpServletRequest request) {
        try {
            //从请求的header中取出当前登录的登录
            SYS_USER user = UserManager.getUserToken(request, userService, unitService, peopleService);
            if (user != null) {
                FormModel model=new FormModel();
                List<SYS_People> fuke=peopleService.selectPeoplesByUnitIdAndDuty(user.getUnitId(),"乡科级副职");
                if (fuke!=null){
                    model.setFuKe(fuke.size());
                }
                List<SYS_People> zhengKe=peopleService.selectPeoplesByUnitIdAndDuty(user.getUnitId(),"乡科级正职");
                if (zhengKe!=null){
                    model.setZhengChu(zhengKe.size());
                }
                List<SYS_People> fuChu=peopleService.selectPeoplesByUnitIdAndDuty(user.getUnitId(),"县处级副职");
                if (fuChu!=null){
                    model.setFuChu(fuChu.size());
                }
                List<SYS_People> zhengChu=peopleService.selectPeoplesByUnitIdAndDuty(user.getUnitId(),"县处级正职");
                if (zhengChu!=null){
    ...
    @ApiOperation(value = "考核统计查询", notes = "考核统计查询", httpMethod = "GET", tags = "考核统计查询接口")
    @GetMapping(value = "/assessmentInfo")
    @ResponseBody
    public String getAssessmentData(HttpServletRequest request) {
        try {
            //从请求的header中取出当前登录的登录
            SYS_USER user = UserManager.getUserToken(request, userService, unitService, peopleService);
            if (user != null) {
